...
def make_return_doc(doctype, source_name,name, target_doc=None):
    ...
    def update_item(source_doc, target_doc, source_parent):
        ...
        if doctype == "Purchase Receipt":
            ...
            returned_qty_map = get_returned_qty_map_for_row(source_doc.name, doctype)
            target_doc.qty = -1 * flt(source_doc.qty - flt(returned_qty_map.get('qty') or 0))
            target_doc.stock_qty = -1 * flt(source_doc.stock_qty - (returned_qty_map.get('stock_qty') or 0))
...
@frappe.whitelist()
def make_sales_return(source_name,name):
    doc=make_return_doc("Delivery Note", source_name,name,target_doc=None)
    p_doc=frappe.get_doc("Purchase Receipt",name)
    doc.save(ignore_permissions=True)
    lst=[]
    for i in p_doc.items:
